...
const coloradix = <N extends string>(color: Color<N>) => {
  return {
    alias: <A extends string>(alias: Alias<A, N>) => {
      return {
        overlay: <O extends boolean>(overlay: O) => {
          return {
            enable: <E extends boolean>(
              enable: E
            ): {
              colors: E extends true ? ColorsResult<A> & (O extends true ? ColorsOverlayResult : {}) : ColorsResult<string>;
